...
def api_pedidos():
    try:
        ...
        col_tipo     = find_col(df, ['tipo'])
        col_envase   = find_col(df, ['envase'])
        col_tcult    = find_col(df, ['t.cult','tcult','cultivo','ecol'])
        col_lote     = find_col(df, ['lote'])
        col_kilos    = find_col(df, ['kilos'])
        col_dest     = find_col(df, ['destino'])
        ...
        # Get unique sorted dates (next 14 days centered on today)
        from datetime import timedelta
        hoy = datetime.now()
        rango = [(hoy + timedelta(days=i)).strftime("%d/%m/%Y") for i in range(-2, 12)]
        fechas_en_datos = df['_fecha'].unique().tolist()
        dias = sorted(set(rango) | set(f for f in fechas_en_datos if f and len(f)==10),
                      key=lambda x: x.split('/')[::-1])

        # Build pedidos list
        pedidos = []
        for _, row in df.iterrows():
            def v(col): return str(row[col]).strip() if col and col in df.columns else ''
            def n(col):
                try: return float(str(row[col]).replace(',','.')) if col else 0
                except: return 0

            pedidos.append({
                'pedido':      v(col_pedido),
                'producto':    v(col_producto),
                'cliente':     v(col_cliente),
                'fecha_salida':df.at[row.name, '_fecha'] if col_fecha else '',
                'palets':      n(col_palets),
                'bultos':      int(n(col_bultos)),
                'kilos':       n(col_kilos),
                'tipo':        v(col_tipo) or 'PEDIDO',
                'envase':      v(col_envase),
                'lote':        v(col_lote),
                't_cult':      v(col_tcult),
                'destino':     v(col_dest),
