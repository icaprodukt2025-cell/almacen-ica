...
def parse_pedido_proveedor_pdf(file_bytes):
    """Parse Pedido a Proveedor PDF - ICA format with doubled characters"""
    import io, re
    pedidos = []
    ...
    # --- Cliente (TABLE 0, row 1) ---
    cliente = ""
    if len(tables) > 0 and len(tables[0]) > 1 and tables[0][1]:
        raw = str(tables[0][1][0] or "")
        cliente = limpiar_doble(raw).split('\n')[0].strip()

    # --- Cabecera pedido (TABLE 1) ---
    ...
    if len(tables) > 1 and len(tables[1]) > 1:
        row = tables[1][1]
        destino        = limpiar_doble(str(row[0] if len(row)>0 else "")).strip()
        ref_cliente    = limpiar_doble(str(row[1] if len(row)>1 else "")).strip()
        ...
        matricula      = limpiar_doble(str(row[5] if len(row)>5 else "")).strip()
        transportista  = limpiar_doble(str(row[6] if len(row)>6 else "")).strip()

    # --- Productos (TABLE 2) ---
    # Headers: Presentación | T.Cult | LOTE | C/C | Envase | Tipo Palet | Kilos | Bul/Plt | Palets | Bultos | Precio | Mar.Material
    if len(tables) > 2 and len(tables[2]) > 1:
        row = tables[2][1]

        def split_col(idx):
            return str(row[idx] if idx < len(row) and row[idx] else "").split("\n")

        presentaciones = split_col(0)
        tipos_cult     = split_col(1)
        lotes          = split_col(2)
        ccs            = split_col(3)   # C/C: I=Intracomunitario, E=Exportacion
        envases        = split_col(4)
        tipos_palet    = split_col(5)
        kilos_list     = split_col(6)
        bul_plt_list   = split_col(7)
        palets_list    = split_col(8)
        bultos_list    = split_col(9)
        ...
        for pres in presentaciones:
            ...
            if not pres or len(pres) < 5:
                continue
            # Skip annotation lines (all caps: instructions, labels)
            if re.match(r'^[A-Z0-9 /\.\-\+\(\)]+$', pres) and len(pres) > 8:
                continue
            ...
            lote       = get(lotes,      prod_idx)
            envase     = get(envases,    prod_idx)
            tipo_cult  = get(tipos_cult, prod_idx)
            tipo_palet = get(tipos_palet,prod_idx)
            cc         = get(ccs,        prod_idx)
            kilos      = get(kilos_list, prod_idx)
            bul_plt    = get(bul_plt_list,prod_idx)

            try:    palets_val = float(get(palets_list, prod_idx).replace(",", "."))
            except: palets_val = 0
            try:    bultos_val = int(get(bultos_list, prod_idx))
            except: bultos_val = 0
