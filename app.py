...
def importar_programa():
    ...
    try:
        # Allow session token OR admin token for Apps Script
        token = request.headers.get('X-Session-Token', '')
        is_admin_token = token == 'ica2025admin'
        is_valid_session = token in sessions if sessions else False
        if not is_valid_session and not is_admin_token:
