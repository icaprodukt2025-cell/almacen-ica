...
def planificacion_diaria():
    try:
        ...
        merma = float(request.args.get('merma', 12)) / 100
        ...
        for _, row in df_dia.iterrows():
            bultos   = to_num(row[col_bultos] if col_bultos else 0)
            palets   = to_num(row[col_palets] if col_palets else 0)
            kg_total = to_num(row[col_kilos]  if col_kilos  else 0)
            producto = str(row[col_producto] if col_producto else '')
            envase   = str(row[col_envase]   if col_envase  else '')
            bul_plt  = to_num(row[col_bul_plt] if col_bul_plt else 0)
...
def parse_pedido_proveedor_pdf(file_bytes):
    ...
    if len(tables) > 2 and len(tables[2]) > 1:
        ...
        def es_nombre_producto(linea):
            l = linea.strip()
            if not l or len(l) < 5: return False
            return bool(re.search(r'[a-z]', l))

        def es_lote(linea):
            return bool(re.match(r'^\d{3,6}$', linea.strip()))

        def es_numero(linea):
            return bool(re.match(r'^[\d.,]+$', linea.strip()))

        def es_envase(linea):
            l = linea.strip()
            return bool(re.match(r'^(IFCO|BLL|EPS|EURO RET|CAJA|FIANZA)', l, re.I)) and len(l) > 3

        def es_tipo_palet(linea):
            l = linea.strip()
            return bool(re.match(r'^(EURO|CHEP|INDUST)', l, re.I))

        def es_tipo_cult(linea):
            return linea.strip() in ('ECOLOGICO', 'CONVENCIONAL', 'I', 'II', 'III', 'ECOLÓGICO')

        # Extract only valid lines per column
        productos   = [l.strip() for l in get_todas(0) if es_nombre_producto(l)]
        lotes       = [l.strip() for l in get_todas(2) if es_lote(l)]
        envases     = [l.strip() for l in get_todas(4) if es_envase(l)]
        tipos_palet_raw = [l.strip() for l in get_todas(5) if es_tipo_palet(l)]
        kilos_l     = [l.strip() for l in get_todas(6) if es_numero(l)]
        bul_plt_raw = [l.strip() for l in get_todas(7) if es_numero(l)]
        palets_l    = [l.strip() for l in get_todas(8) if es_numero(l)]
        bultos_l    = [l.strip() for l in get_todas(9) if re.match(r'^\d+$', l.strip())]
        tipos_cult_raw = [l.strip() for l in get_todas(1) if es_tipo_cult(l)]
        ccs_raw     = [l.strip() for l in get_todas(3) if l.strip() in ('I','II','III','E','C')]
        # bultos already clean integers
