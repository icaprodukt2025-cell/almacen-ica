...
def cargas():
    ...
    for _, row in df_cargas.iterrows():
        fecha = str(row.iloc[0]).strip() if len(row) > 0 else ""
        if not fecha.startswith(hoy):
            continue
        pedido = str(row.get("Pedido", "") or row.get("N? Pedido", "") or "").strip()
        matricula = str(row.get("Matricula", "") or row.get("Matricula", "") or "").strip()
        transportista = str(row.get("Transportista", "") or "").strip()
        mercancia = str(row.get("Mercancia", "") or row.get("Mercancia", "") or "").strip()
        destino = str(row.get("Destino/Referencia", "") or row.get("Destino", "") or "").strip()
        palets = str(row.get("Palets", "") or row.get("Palet", "") or "").strip()
        ...
        items.append({
            ...
            "transportista": transportista if transportista != "nan" else "",
            "mercancia": mercancia if mercancia != "nan" else "",
            "destino": destino if destino != "nan" else "",
            "palets": palets if palets != "nan" else "",
            "observaciones": obs if obs != "nan" else "",
            "fecha": fecha
        })
...
def guardar_confeccion():
    try:
        ...
        # Crear hoja Confecciones si no existe
        ...
        if "Confecciones" not in sheet_names:
            ...
            service.spreadsheets().values().update(
                ...
                body={"values": [["Fecha", "Hora", "Pedido", "Producto", "Cliente", "Palet", "Lote", "Kg Usados", "Kg Destrio"]]}
            ).execute()

        # Add one row per lote
        rows = []
        for lote in lotes:
            rows.append([
                fecha, hora, pedido, producto, cliente, palet,
                lote.get("lote", ""),
