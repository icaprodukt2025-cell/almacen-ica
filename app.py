...
def parse_pedido_proveedor_pdf(file_bytes):
    """Parse Pedido a Proveedor PDF - uses fixed table indices"""
    import io, re
    pedidos = []
    ...
    # Num pedido from text (e.g. "Pedido:22225500" -> 2250)
    num_pedido = ""
    m = re.search(r'Pedido:\s*([\d]+)', text)
    if m:
        num_pedido = limpiar_doble(m.group(1))

    # TABLE 0: ICA company + cliente
    cliente = ""
    if len(tables) > 0 and len(tables[0]) > 1:
        cliente = limpiar_doble(str(tables[0][1][0] if tables[0][1] else "")).strip()

    # TABLE 1: header row (Bestellnummer) + data row
    fecha_salida = fecha_llegada = transportista = matricula = destino = ""
    if len(tables) > 1 and len(tables[1]) > 1:
        data_row = tables[1][1]
        destino      = limpiar_doble(str(data_row[0] if len(data_row)>0 else "")).strip()
        fecha_salida = limpiar_fecha_doble(str(data_row[3] if len(data_row)>3 else ""))
        fecha_llegada= limpiar_fecha_doble(str(data_row[4] if len(data_row)>4 else ""))
        matricula    = limpiar_doble(str(data_row[5] if len(data_row)>5 else "")).strip()
        transportista= limpiar_doble(str(data_row[6] if len(data_row)>6 else "")).strip()

    # TABLE 2: product header + product data (merged cells with \n)
    if len(tables) > 2 and len(tables[2]) > 1:
        prod_row = tables[2][1]
        presentaciones = str(prod_row[0]).split('\n') if prod_row[0] else []
        cultivos       = str(prod_row[1]).split('\n') if len(prod_row)>1 else []
        lotes          = str(prod_row[2]).split('\n') if len(prod_row)>2 else []
        envases        = str(prod_row[4]).split('\n') if len(prod_row)>4 else []
        palets_list    = str(prod_row[8]).split('\n') if len(prod_row)>8 else []
        bultos_list    = str(prod_row[9]).split('\n') if len(prod_row)>9 else []

        prod_idx = 0
        for pres in presentaciones:
            pres = pres.strip()
            if not pres or len(pres) < 5: continue
            # Skip annotation lines (all caps, no lowercase)
            if re.match(r'^[A-Z0-9 /\.\-\+]+$', pres) and len(pres) > 10: continue
            if re.match(r'^[\d\+]+$', pres): continue

            lote   = lotes[prod_idx].strip()   if prod_idx < len(lotes)   else ""
            envase = envases[prod_idx].strip()  if prod_idx < len(envases) else ""
            try:    palets_val = float((palets_list[prod_idx] if prod_idx<len(palets_list) else "0").replace(',','.'))
            except: palets_val = 0
            try:    bultos_val = int(bultos_list[prod_idx] if prod_idx<len(bultos_list) else "0")
            except: bultos_val = 0
    ...
    return {'pedidos': pedidos, 'tipo': 'pedido_proveedor'}


def parse_pedido_proveedor_pdf(file_bytes):
    """Parse Pedido a Proveedor PDF format"""
    import io, re
    pedidos = []
    ...
    # Extract pedido number from text
    num_pedido = ""
    m = re.search(r'Pedido:\s*([\d]+)', text)
    if m:
        raw = m.group(1)
        # Fix doubled: 22225500 -> 2250
        num_pedido = limpiar_doble(raw)

    # Extract header info from tables
    cliente = ""
    fecha_salida = ""
    fecha_llegada = ""
    transportista = ""
    ...
    for table in tables:
        for i, row in enumerate(table):
            if not row:
                continue
            # Header row with Bestellnummer
            if len(row) >= 7 and row[0] and 'bestellnummer' in str(row[0]).lower():
                continue
            # Data row with EDEKA/destino info
            if len(row) >= 7 and row[0] and ('RROOII' in str(row[0]) or 'EDEKA' in str(row[0]).upper() or limpiar_doble(str(row[0])).strip()):
                destino = limpiar_doble(str(row[0])).strip()
                fecha_salida = limpiar_fecha_doble(str(row[3])) if len(row) > 3 else ""
                fecha_llegada = limpiar_fecha_doble(str(row[4])) if len(row) > 4 else ""
                matricula = limpiar_doble(str(row[5])).strip() if len(row) > 5 else ""
                transportista = limpiar_doble(str(row[6])).strip() if len(row) > 6 else ""
                # Try to get cliente from header text
                m_cli = re.search(r'(EDEKA[^\n]+)', text)
                if m_cli:
                    cliente = limpiar_doble(m_cli.group(1)).strip()

            # Product rows - detect by Presentacion header
            if len(row) >= 10 and row[0] and 'presentaci' in str(row[0]).lower():
                continue
            # Product data rows with multiple subrows (pdfplumber merges cells)
            if len(row) >= 10 and row[8] and row[9]:  # has Palets and Bultos
                presentaciones = str(row[0]).split('\n')
                cultivos = str(row[1]).split('\n')
                lotes = str(row[2]).split('\n')
                ccs = str(row[3]).split('\n')
                envases = str(row[4]).split('\n')
                palets_list = str(row[8]).split('\n')
                bultos_list = str(row[9]).split('\n')

                for j, pres in enumerate(presentaciones):
                    pres = pres.strip()
                    if not pres or len(pres) < 5:
                        continue
                    # Skip instruction lines (all caps annotations)
                    if pres.isupper() and len(pres) > 10:
                        continue
                    # Skip lines that are clearly not product names
                    if re.match(r'^[\d\+]+$', pres):
                        continue
                    lote = lotes[j].strip() if j < len(lotes) else ""
                    envase = envases[j].strip() if j < len(envases) else ""
                    palets_raw = palets_list[j].strip() if j < len(palets_list) else "0"
                    bultos_raw = bultos_list[j].strip() if j < len(bultos_list) else "0"

                    try:
                        palets_val = float(palets_raw.replace(',', '.'))
                    except:
                        palets_val = 0
                    try:
                        bultos_val = int(bultos_raw)
                    except:
                        bultos_val = 0
    ...
    try:
        ...
        for p in data['pedidos']:
            ...
            def set_col(name, val):
                ...
                if idx >= 0:
                    row[idx] = val
            # For pedido_proveedor, use per-row fields; for programa use global ones
            p_producto = p.get('producto') or producto
            p_cliente = p.get('cliente') or cliente or marca
            p_lote = p.get('lote', '') or ''
            p_envase = p.get('envase', '') or ''
            p_transportista = p.get('transportista', '') or ''
            p_matricula = p.get('matricula', '') or ''
            p_fecha = p.get('fecha_salida', '') or ''
            set_col('pedido', p['pedido'])
            set_col('fecha', p_fecha)
            set_col('salida', p_fecha)
            set_col('producto', p_producto)
            set_col('presentacion', p_producto)
            set_col('cliente', p_cliente)
            set_col('bultos', p['bultos'])
            set_col('palet', p.get('palets', ''))
            set_col('palets', p.get('palets', ''))
            set_col('carga', p.get('carga', ''))
