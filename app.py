...
def parse_pedido_proveedor_pdf(file_bytes):
    """Parse Pedido a Proveedor PDF - ICA format"""
    import io, re
    pedidos = []
    ...
    # --- Pedido number (from text, no doubling here) ---
    num_pedido = ""
    m = re.search(r'Pedido:\s*(\d+)', text)
    if m:
        raw = m.group(1)
        num_pedido = limpiar_doble(raw) if tiene_dobles(raw) else raw

    # --- Cliente (TABLE 0 row 1 = cliente/destino block) ---
    cliente = ""
    if len(tables) > 0 and len(tables[0]) > 1 and tables[0][1]:
        raw = str(tables[0][1][0] or "")
        cliente = limpiar_si_doble(primera_linea(raw))

    # --- Cabecera (TABLE 1) ---
    # Headers: Bestellnummer | Numero Ref. | Fecha Pedido | Fecha Salida | Fecha Llegada | Matrícula | Transportista
    destino = ref_cliente = fecha_pedido = fecha_salida = fecha_llegada = matricula = transportista = ""
    if len(tables) > 1 and len(tables[1]) > 1:
        row = tables[1][1]
        destino        = limpiar_si_doble(primera_linea(str(row[0] if len(row)>0 else "")))
        ref_cliente    = limpiar_si_doble(primera_linea(str(row[1] if len(row)>1 else "")))
        fecha_pedido   = limpiar_fecha_doble(str(row[2] if len(row)>2 else ""))
        ...
        matricula      = limpiar_si_doble(primera_linea(str(row[5] if len(row)>5 else "")))
        transportista  = limpiar_si_doble(primera_linea(str(row[6] if len(row)>6 else "")))
        # If destino empty, use cliente from TABLE 0
        if not destino:
            destino = cliente
    ...
    if len(tables) > 2 and len(tables[2]) > 1:
        ...
        def get_primera(idx):
            return primera_linea(str(row[idx] if idx < len(row) and row[idx] else ""))

        def get_todas(idx):
            if idx >= len(row) or not row[idx]: return []
            return [l.strip() for l in str(row[idx]).split("\n") if l.strip()]

        # Product names: only lines that look like product names (mixed case, >5 chars, not all caps)
        presentaciones_raw = get_todas(0)
        productos = []
        for l in presentaciones_raw:
            if len(l) < 5: continue
            # Skip all-caps annotation lines
            if re.match(r'^[A-Z0-9 /\.\-\+\(\)=,]+$', l) and len(l) > 8: continue
            if re.match(r'^[\d\+\.]+$', l): continue
            productos.append(l)

        lotes_raw       = get_todas(2)
        envases_raw     = get_todas(4)
        tipos_palet_raw = get_todas(5)
        kilos_raw       = get_todas(6)
        bul_plt_raw     = get_todas(7)
        palets_raw      = get_todas(8)
        bultos_raw      = get_todas(9)
        tipos_cult_raw  = get_todas(1)
        ccs_raw         = get_todas(3)

        # Filter lotes/envases to only numeric/short values (exclude cartel contamination)
        def limpiar_lista(lst, es_numero=False):
            clean = []
            for v in lst:
                if es_numero:
                    if re.match(r'^[\d.,]+$', v): clean.append(v)
                else:
                    if len(v) <= 30 and not re.match(r'^[A-Z ]{10,}$', v): clean.append(v)
            return clean

        lotes     = limpiar_lista(lotes_raw)
        envases   = limpiar_lista(envases_raw)
        palets_l  = limpiar_lista(palets_raw, True)
        bultos_l  = limpiar_lista(bultos_raw, True)
        kilos_l   = limpiar_lista(kilos_raw)
        # Normalize Spanish thousands separator in bultos
        bultos_raw_norm = []
        for v in bultos_l:
            try: bultos_raw_norm.append(str(int(v.replace('.','').replace(',',''))))
            except: bultos_raw_norm.append(v)
        bultos_l = bultos_raw_norm

        for i, pres in enumerate(productos):
            lote       = lotes[i].strip()      if i < len(lotes)     else ""
            envase     = envases[i].strip()    if i < len(envases)   else ""
            tipo_palet = tipos_palet_raw[i].strip() if i < len(tipos_palet_raw) else ""
            tipo_cult  = tipos_cult_raw[i].strip()  if i < len(tipos_cult_raw)  else ""
            kilos      = kilos_l[i].strip()    if i < len(kilos_l)   else ""

            try:    palets_val = float((palets_l[i] if i < len(palets_l) else "0").replace(",", "."))
            except: palets_val = 0
            try:
                braw = (bultos_l[i] if i < len(bultos_l) else "0").replace(".", "").replace(",", "")
                bultos_val = int(braw)
            except: bultos_val = 0
            ...
            if bultos_val > 0:
                pedidos.append({
                    "pedido":        num_pedido,
                    "cliente":       cliente,
                    "destino":       destino or ref_cliente,
                    "referencia":    ref_cliente,
                    "producto":      pres,
                    "tipo":          tipo_cult,
                    "lote":          lote,
                    "envase":        envase,
...
def importar_programa():
    ...
    try:
        ...
        for p in data['pedidos']:
            ...
            p_fecha_llegada= p.get('fecha_llegada','') or ''
            p_fecha_pedido = p.get('fecha_pedido', '') or ''
            p_destino      = p.get('destino',      '') or ''
            p_referencia   = p.get('referencia',   '') or ''
            p_tipo         = p.get('tipo',         '') or ''
            p_tipo_palet   = p.get('tipo_palet',   '') or ''
            p_kilos        = p.get('kilos',        '') or ''
            set_col('pedido',        p['pedido'])
            set_col('fecha',         p_fecha)
            set_col('salida',        p_fecha)
            set_col('fecha salida',  p_fecha)
